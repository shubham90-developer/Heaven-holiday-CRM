...
import { useCreateSightseeingMutation } from "../../../../../../Redux/sightSeeingApi";
...
// ─── Section config — maps section title to formState key ─────────────────────
...
const AddSightseeing = () => {
  ...
  // RTK mutation
  const [createSightseeing, { isLoading, isError, error }] =
  ...
  const updateOperatingTime = (
    ...
    const updated = form.operatingTimes.map((t, i) =>
      i === index ? { ...t, [field]: value } : t,
    );
    setField("operatingTimes", updated);
  };
  ...
  const handleSubmit = async () => {
    try {
      // Build FormData (backend expects multipart because of image)
      ...
      // Optional string fields — only append if non-empty
      ...
      // Rich-text (html strings from Quill)
      ...
      await createSightseeing(formData).unwrap();
      alert("Sightseeing created successfully!");
      router.push("/my-inventory/sightseeing");
    } catch (err) {
      console.error("Failed to create sightseeing:", err);
  ...
  return (
    <>
      <Card className="p-3">
        ...
        <Row>
          ...
          {showAddDetail && (
            <AddDetailModal
              ...
              onApply={(name: string, lat: string, lng: string) => {
        ...
        {/* ── Rich Text Sections (React Quill) ──────────────────────────────── */}
        ...
        {isError && (
          <div className="text-danger mt-2" style={{ fontSize: "11px" }}>
            Error saving sightseeing. Please check your inputs and try again.
          </div>
        )}
