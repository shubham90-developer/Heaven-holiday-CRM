...
const B2BCustomers = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetAllLeadsQuery({ type: "B2B", page, limit: 20 });
  ...
  const handleToggleStatus = async (lead: ILead) => {
    const newStatus = lead.archived ? "inProcess" : "archived";
    await updateLead({
      id: lead._id,
      body: { status: newStatus as any, archived: !lead.archived },
    });
    refetch();
  };
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((lead) => (
                    ...
                    >
                      ...
                      <td>
                        <span
                          className={`badge ${lead.archived ? "bg-secondary" : "bg-success"}`}
                        >
